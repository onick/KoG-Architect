...
| `/kog-delegate` | Generate precise delegation prompt for a task |
...
These are NOT separate subagents — they are role personas the architect switches into depending on the task domain. Applied one at a time, sequentially.
...
│           ├── SKILL.md         #   Main brain
│           └── references/      #   8 domain docs + 5 delegation templates
│
...
| Delegation Templates | `references/*-agent.md` | `references/*-agent.md` |
